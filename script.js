...
//API
const API_KEY = "AIzaSyDrfiecALDf5oVqvwTkeLLowf544NC3Zvo";
...
const createMsgElemment = (content, ...classes) => {
...
`;
    ...
    const userMsgDiv = createMsgElemment(userMsgHTML, "user-message");
    userMsgDiv.querySelector(".message-text").textContent = userMessage;
    chatsContainer.appendChild(userMsgDiv);
    scrollToBottom();

    //gemini ka image yaha pe hn 
    setTimeout(() => {
        const botMsgHTML = `<img src="gemini.svg" class="avatar"><p class="message-text">Just a Sec...</p>`;
        const botMsgDiv = createMsgElemment(botMsgHTML, "bot-message", "loading");
...
document.addEventListener("click", ({ target }) => {
    ...
    const shouldHide = target.classList.contains("prompt-input") || (fileUploadWrapper.classList.contains
        ("hide-controls") && (target.id === "add-file-btn" || target.id === "stop-response-btn"));
    fileUploadWrapper.classList.toggle("hide-controls", shouldHide);
})
//delete
